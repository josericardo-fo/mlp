...
def main():
    ...
    parser.add_argument(
        "--validation-split", type=float, default=0.1, help="Validation split ratio"
    )
    parser.add_argument(
        "--weight-init",
        type=str,
    ...
    parser.add_argument(
        "--no-bias", action="store_false", dest="use_bias", help="Disable bias"
    )
    parser.add_argument(
        "--model-path", type=str, default="model.pkl", help="Path to save the model"
    )
    ...
    print(
        f"Creating MLP with {args.input_size} inputs, {args.hidden_size} hidden neurons, {args.output_size} outputs"
    )
    mlp = MLP(
